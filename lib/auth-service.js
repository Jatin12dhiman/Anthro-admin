...
/**
 * Pehla registered user = Super Admin (admin panel bootstrap),
 * baaki sab = User. (PRD: roles DB-driven.)
 */
export async function defaultRoleForNewUser() {
  const count = await User.estimatedDocumentCount();
  const roleName = count === 0 ? "Super Admin" : "User";
  return Role.findOne({ name: roleName });
}

/** API ke liye safe user shape — password_hash kabhi nahi. */
export function publicUser(user) {
  const roles = (user.role_ids || []).map((r) => (r && r.name ? r.name : String(r)));
  return {
    id: String(user._id),
    name: user.name,
    email: user.email,
